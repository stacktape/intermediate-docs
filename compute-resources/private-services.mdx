...
- **`service-connect` (default):** Uses ECS Service Connect. The service is only accessible to other ECS-based resources like [web services](/compute-resources/web-services), [worker services](/compute-resources/worker-services), and [multi-container workloads](/compute-resources/multi-container-workloads).
...
For more details, see the sections on [Load Balancing](#load-balancing) and [Connecting to a Private Service](#connecting-to-a-private-service).
...
- **`port`:** `3000` by default, but can be customized using the [port](#port) property.
...
- **[stacktape-image-buildpack](/configuration/packaging#stacktape-image-buildpack-private-service):** Automatically packages your code without needing a Dockerfile.
- **[external-buildpack](/configuration/packaging#external-buildpack-private-service):** Uses external buildpacks to create an image.
- **[custom-dockerfile](/configuration/packaging#custom-dockerfile-private-service):** Builds an image from your own Dockerfile.
- **[prebuilt-images](/configuration/packaging#prebuilt-image-private-service):** Uses an existing image from a container registry.
...
Private services are connected to this default _VPC_ by default, allowing them to communicate with other VPC-based resources without extra configuration.
