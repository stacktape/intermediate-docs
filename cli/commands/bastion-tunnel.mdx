...
- Your stack must contain `bastion` resource type to perform this command.
- Opening tunnels can be useful for accessing resources protected in VPC.
- If you do not specify `--bastionResource` argument when using this command, random bastion from your stack is used.
- After tunnels are open, command will print the tunneled endpoints to the terminal.
- Session is established using secured SSM connection.
- For more information refer to [bastion docs](https://docs.stacktape.com/bastion-servers/#creating-bastion-tunnel).
